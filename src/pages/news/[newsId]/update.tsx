...
import type { AppRouter } from "@/src/utils/api";

type RouterOutput = inferRouterOutputs<AppRouter>;
type DetailedNews = RouterOutput["post"]["getAllPostsVisibleToUser"][number];

const UpdateNewsForm = (props: { news: DetailedNews }) => {
  const getSelf = api.user.getSelf.useQuery();

  const [post, setPost] = useState({
    title: props.news.title,
    content: props.news.content,
    lowestVisibleRole: props.news.lowestVisibleRole,
  });
...
const UpdateNews = () => {
  ...
  const getPost = api.post.getOne.useQuery({
  ...
  if (getPost.isLoading || !getPost.data) {
    return <LoadingPage />;
  }
      ...
      <main>
        ...
        <div className="flex w-full justify-center">
          <main className="w-full sm:max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg">
            <UpdateNewsForm news={getPost.data} />
          </main>
        </div>
        <Footer />
      </main>
